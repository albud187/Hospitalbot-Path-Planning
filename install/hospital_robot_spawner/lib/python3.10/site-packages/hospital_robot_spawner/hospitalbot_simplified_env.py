...
class HospitalBotSimpleEnv(RobotController, gym.Env):
    """
    ...
    def __init__(self):
        ...
        self.action_space = spaces.Box(low=np.array([0, -self._angular_velocity]), high=np.array([self._linear_velocity, self._angular_velocity]), dtype=np.float32)
        ...
        self.observation_space = spaces.Box(low=np.array([-100, -math.pi]), high=np.array([100, math.pi]), dtype=np.float32)
